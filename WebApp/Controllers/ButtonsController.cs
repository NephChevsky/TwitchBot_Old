        ...
        private async Task<string> GetButtonValue(string name)
        {
            ...
            else if (name == "top_cheers_daily" || name == "top_cheers_monthly" || name == "top_cheers_total")
            {
                using (TwitchDbContext db = new())
                {
                    ...
                    if (topCheerer != null)
                    {
                        Viewer viewer = await _api.GetOrCreateUserById(topCheerer.Owner);
                        if (viewer != null)
                        {
            ...
            else if (name == "last_subscriber")
            {
                using (TwitchDbContext db = new())
                {
                    ...
                    if (sub != null)
                    {
                        Viewer viewer = await _api.GetOrCreateUserById(sub.Owner);
                        if (viewer != null)
                        {
            ...
            else if (name == "last_subscription_gifter")
            {
                using(TwitchDbContext db = new())
                {
                    ...
                    if (subgift != null)
                    {
                        Viewer viewer = await _api.GetOrCreateUserById(subgift.GifterId);
                        if (viewer != null)
                        {
                            return viewer.DisplayName;
						}
            ...
            else if (name == "top_subscription_gifter_daily" || name == "top_subscription_gifter_monthly" || name == "top_subscription_gifter_total")
            {
                using (TwitchDbContext db = new())
                {
                    ...
                    {
                        Viewer viewer = await _api.GetOrCreateUserById(topSubGifter.GifterId);
                        if (viewer != null)
                        {
            ...
            else if (name == "longest_subscriber")
            {
                using (TwitchDbContext db = new())
                {
                    ...
                    Viewer viewer = await _api.GetOrCreateUserById(subers[0].Owner);
                    if (viewer != null)
                    {
                        return viewer.DisplayName;
					}
            ...
            else if (name == "last_cheer")
            {
                using (TwitchDbContext db = new())
                {
                    ...
                    if (lastCheer != null)
                    {
                        Viewer viewer = await _api.GetOrCreateUserById(lastCheer.Owner);
                        if (viewer != null)
                        {
